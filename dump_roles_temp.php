...
echo "INSERT INTO roles (id, name, cooldown, description, rules, active, fill, amount, icon_path, sichtbar, befragen, auto_eintrag, is_killer, sort_order) VALUES\n";

$lines = [];
foreach ($roles as $r) {
    $lines[] = sprintf(
        "(%d, %s, %d, %s, %s, %d, %d, %d, %s, %d, %d, %d, %d, %d)",
        $r['id'], $esc($r['name']), (int)$r['cooldown'],
        $esc($r['description']), $esc($r['rules']),
        (int)$r['active'], (int)$r['fill'], (int)$r['amount'],
        $esc($r['icon_path']),
        (int)$r['sichtbar'], (int)$r['befragen'],
