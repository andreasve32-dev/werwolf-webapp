...
// Wiederverwendbare Render-Funktionen für stats.php — genutzt sowohl beim
// initialen Seitenaufbau als auch von api/game.php (get_stats) fürs Polling.

if (!function_exists('stats_compute_state')) {
function stats_compute_state(): array {
    ...
    return compact(
        'totals', 'causePieData', 'phasePieData', 'roundBarData', 'accuseBarData',
        'allPlayers', 'playerDetails', 'hasData'
    );
}
}

if (!function_exists('render_stats_content')) {
function render_stats_content(array $s): string {
    extract($s);
    ob_start();
    ?>
    <?php if (!$hasData): ?>
